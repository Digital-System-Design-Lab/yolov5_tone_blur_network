...
def gaussian_blur(image, kernel_size, sigma):
    """
    이미지에 가우시안 블러를 적용합니다. (이소트로픽 가우시안 블러)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        if 'dynamic_luminance_network' in ckpt: #ckpt(체크포인트) 안에 downscaling_network 상태가 있을때만 로드 없으면 로드x 
    ...
    if pretrained:
        if resume:
            best_fitness, start_epoch, epochs = smart_resume(ckpt, optimizer, dln_optimizer, ema, weights, epochs, resume)#dfm_optimizer 추가 0226
        del ckpt, csd
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp): #amp: 자동 혼합 정밀도 -> 그냥 계산 효율성, 성능 향상시키는 것 인듯..
                ...
                dynamic_luminance_weights = torch.clamp(dynamic_luminance_weights, min=0, max=5)#0302추가 cliping 
                # imgs는 원본이미지
                resized_imgs = []
                for img, weights in zip(imgs, dynamic_luminance_weights):#batch size 2이상에도 돌아가게 구현
                    weights = weights.to(img.device)
                    resized_img = reinhard_tone_mapping(img.unsqueeze(0), weights[0])#reinhard_tone_mapping 들어갈때 Batch size 추가
                ...
                resized_imgs_to_ycbcr = TJ_ycbcr.to_ycbcr(resized_imgs, 1.0) # 0405코드 추가 // RGB 이미지를 YCbcr로 바꾸기(shape은 동일/ 단 픽셀은 [0,1] 값을 가짐)
                ...
                loss, loss_items = compute_loss(pred, targets.to(device), dynamic_luminance_weights)  # loss scaled by batch_size // pred결과와 target 결과를 기반으로 loss 계산(yolov5)
            ...
            if ni - last_opt_step >= accumulate:
                ...
                torch.nn.utils.clip_grad_norm_(dynamic_luminance_network.parameters(), max_norm=10.0) # 0303추가 0305수정
            ...
            if RANK in {-1, 0}:
                ...
                pbar.set_description(
                    ("%11s" * 2 + "%11.4g" * 7)
                    % (f"{epoch}/{epochs - 1}", mem, *mloss, mtotal_loss.item(), mbpp_loss.item(),targets.shape[0], imgs.shape[-1])#mbpp, mtotal_loss 추가 #pbar에 나타내는듯
                )
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = { #checkpoint 인듯
                    ...
                    "dynamic_luminance_network": deepcopy(dynamic_luminance_network.state_dict()),  # Save downscaling_network state syh edit 저장해야함
                }
