...
class DynamicLoss(nn.Module):
    ...
    def forward(self, input, target, eps=1e-5):
        ...
        if self.raw:
            ...
            target_left2, target_right2, target_up2, target_down2 = (
                target_left2.view(-1),
                target_right2.view(-1),
                target_up2.view(-1),
                target_down2.view(-1),
            )
        ...
        if self.one_hot:
            ...
            if self.raw:
                target_onehot2 = (
                    one_hot(target_left, input.size(1))
                    + one_hot(target_right, input.size(1))
                    + one_hot(target_up, input.size(1))
                    + one_hot(target_down, input.size(1))
                    + one_hot(target_left2, input.size(1))
                    + one_hot(target_right2, input.size(1))
                    + one_hot(target_up2, input.size(1))
                    + one_hot(target_down2, input.size(1))
                )
