...
class Discriminator(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(num_classes, 16, 5, stride=1, padding=2), nn.LeakyReLU(inplace=True), nn.InstanceNorm2d(16)
        )

        # input 512x512x16  output 256x256x32
        self.conv2 = nn.Sequential(
            nn.Conv2d(16, 32, 5, stride=2, padding=2), nn.LeakyReLU(inplace=True), nn.InstanceNorm2d(32)
        )

        # input 256x256x32  output 128x128x64
        self.conv3 = nn.Sequential(
            nn.Conv2d(32, 64, 5, stride=2, padding=2), nn.LeakyReLU(inplace=True), nn.InstanceNorm2d(64)
        )

        # input 128x128x64  output 64x64x128
        self.conv4 = nn.Sequential(
            nn.Conv2d(64, 128, 5, stride=2, padding=2), nn.LeakyReLU(inplace=True), nn.InstanceNorm2d(128)
        )

        # input 64x64x128  output 32x32x128
        # the output of this layer we need layers for global features
        self.conv5 = nn.Sequential(
            nn.Conv2d(128, 128, 5, stride=2, padding=2), nn.LeakyReLU(inplace=True), nn.InstanceNorm2d(128)
        )
        ...
        self.conv7 = nn.Sequential(nn.Conv2d(128, 1, 16), nn.LeakyReLU(inplace=True))
