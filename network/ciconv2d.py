...
from network.util_filters import tanh_range
...
class CIConv2d(nn.Module):
    ...
    def forward(self, batch, scale=0.0):
        self.scale = tanh_range(-2.5, 2.5)(scale)
        # self.scale = scale
        ...
        # self.scale = torch.clamp(self.scale, min=-2.5, max=2.5)
