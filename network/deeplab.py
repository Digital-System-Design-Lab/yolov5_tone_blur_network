...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes, dgf, dgf_r, dgf_eps):
        self.inplanes = 64
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.dgf:
            ...
            x2 = F.interpolate(x2, im.size()[2:], mode="bilinear", align_corners=True)
...
def Deeplab(num_classes=21, dgf=False, dgf_r=4, dgf_eps=1e-2):
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], num_classes, dgf, dgf_r, dgf_eps)
    return model
