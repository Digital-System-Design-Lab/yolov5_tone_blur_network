...
class PSPModule(nn.Module):
    ...
    def __init__(self, features, out_features=512, sizes=(1, 2, 3, 6)):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(
                features + len(sizes) * out_features, out_features, kernel_size=3, padding=1, dilation=1, bias=False
            ),
    ...
    def forward(self, feats):
        ...
        priors = [
            F.upsample(input=stage(feats), size=(h, w), mode="bilinear", align_corners=True) for stage in self.stages
        ] + [feats]
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, dgf, dgf_r, dgf_eps, with_r=False):
        ...
        self.head = nn.Sequential(
            PSPModule(2048, 512), nn.Conv2d(512, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
        )
