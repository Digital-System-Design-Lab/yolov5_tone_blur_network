...
from network.util_filters import lerp, rgb2lum, tanh_range
...
class Filter(nn.Module):
    ...
    def get_mask(self, img, mask_parameters):
        ...
        with tf.name_scope(name="mask"):
            ...
            # Ax + By + C * L + D
            inp = (
                grid[:, :, :, 0, None] * mask_parameters[:, None, None, 0, None]
            ...
            mask = (
                mask
                * (mask_parameters[:, None, None, 5, None] / filter_input_range * 0.5 + 0.5)
                * (1 - self.cfg.minimum_strength)
    ...
    def visualize_mask(self, debug_info, res):
        return cv2.resize(
            debug_info["mask"] * np.ones((1, 1, 3), dtype=np.float32), dsize=res, interpolation=cv2.cv2.INTER_NEAREST
        )
...
class ToneFilter(Filter):
    ...
    def process(self, img, param, defog, IcA):
        ...
        for i in range(self.cfg.curve_steps):
            total_image += (
                torch.clamp(img - 1.0 * i / self.cfg.curve_steps, 0, 1.0 / self.cfg.curve_steps) * param[:, i, :, :]
            )
