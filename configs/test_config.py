...
def get_arguments():
    ...
    parser.add_argument("--gpu_id", dest="gpu_id", type=str, default="2", help="if use gpu, use gpu device id")
    parser.add_argument("--model", type=str, default=MODEL, help="Model Choice (DeeplabMulti/DeeplabVGG/Oracle).")
    parser.add_argument("--NIGHTCITY_FLAG", dest="NIGHTCITY_FLAG", type=bool, default=True, help="whether nightcity  ")
    parser.add_argument(
        "--CITYSCAPES_FLAG", dest="CITYSCAPES_FLAG", type=bool, default=False, help="whether  cityscapes "
    )
    ...
    parser.add_argument(
        "--restore-from-light", type=str, default=RESTORE_FROM_LIGHT, help="Where restore model parameters from."
    )
